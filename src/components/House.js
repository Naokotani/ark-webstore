...
const House = (props) => {
  ...
  function findComponentType(arr) {
    if (props.node) {
      const comp = arr.filter((comp) => comp.node._id === props.node._ref);
      return comp[0].node;
    } else {
      return {
        type: "All",
  ...
  function findSpecificHouse(comp, houseArr) {
    const titles = comp.specific.map((e) => {
      return e.title;
    });
    return houseArr.filter((house) =>
      titles.some((title) => title === house.node.title)
    );
  }
  ...
  function createHouseArray(comp) {
    ...
    if (
      comp.type === "All" ||
      (comp.type === "Random" && h.length >= comp.number)
    ) {
  ...
  useEffect(() => {
    const comp = findComponentType(data.allSanityHouseComponent.edges);
    const houses = createHouseArray(comp);
    setHousesArr(houses);
  }, [data]);

  const linkURL = !props.page ? "houses" : false;
            ...
            >
              ...
              <section>
                <header className="flex underline">
                  ...
                  {!props.page && <Link to="houses">See All Houses</Link>}
                </header>
                <h5>{house.node.address}</h5>
                <BlockContent
                  blocks={house.node._rawBody}
